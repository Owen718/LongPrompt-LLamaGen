...
def main(args):
    ...
    t5_xxl.model = torch.compile(t5_xxl.model,mode="reduce-overhead")


    # Setup data:
    print(f"Dataset is preparing...")
    dataset = SimpleCustomDataset(args.data_path,length=500000)#, args.data_start, args.data_end, args.caption_key, args.trunc_caption)
    ...
    t5_xxl.model.to(device)
    from tqdm import tqdm
    for caption, code_dir, code_name in tqdm(loader):
        if os.path.exists(os.path.join(args.t5_path, code_dir[0], '{}.npy'.format(code_name[0]))):
            continue
...
if __name__ == "__main__":
    ...
    parser.add_argument("--t5_model_path", type=str, default='/home/yetian/Project/LlamaGen/pretrained_models/t5-ckpt')
    parser.add_argument("--t5-path", type=str, default='/home/yetian/Project/LlamaGen/t5_embeddings')
