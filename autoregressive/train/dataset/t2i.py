...
class Text2ImgDataset(Dataset):
    def __init__(self, args, transform):
       ...
       # collect valid jsonl file path
        ...
        # Collect valid t5 embedding files
        valid_t5_files = glob(os.path.join(args.t5_feat_path, '*', '*.npy'))
        
        for t5_file in valid_t5_files:
            # Extract subfolder and filename
            subfolder, filename = os.path.split(os.path.relpath(t5_file, args.t5_feat_path))
            code_name = os.path.splitext(filename)[0]

            # Check for image file with different extensions
            img_extensions = ['.jpg', '.jpeg', '.png', '.webp']
            img_path = None
            for ext in img_extensions:
                potential_img_path = os.path.join(args.data_path, subfolder, f"{code_name}{ext}")
                if os.path.isfile(potential_img_path):
                    img_path = potential_img_path
                    break
            
            # If an image file is found, add it to the list
            if img_path:
                img_path_list.append((img_path, subfolder, code_name))
        ...
        self.t5_feat_path = args.t5_feat_path
        self.short_t5_feat_path = None #args.short_t5_feat_path
        self.t5_feat_path_base = self.t5_feat_path.split('/')[-1]
        if self.short_t5_feat_path is not None:
            self.short_t5_feat_path_base = self.short_t5_feat_path.split('/')[-1]
        else:
        ...
        latent_size = args.image_size // args.downsample_size
        self.code_len = latent_size ** 2
        self.t5_feature_max_len = args.cls_token_num #500
        self.t5_feature_dim = 2048
        self.max_seq_length = self.t5_feature_max_len + self.code_len
...
#     data_path='/home/yetian/Datasets/synthetic-dataset-1m-dalle3-high-quality-captions/synthetic-dataset-1m-dalle3-high-quality-captions/data/extracted_data',
#     t5_feat_path='/home/yetian/Project/LlamaGen/t5_embeddings',
#     image_size=512,
#     downsample_size=16,
# )

# dataset = Text2ImgDataset(args, None)
# for data in dataset:
#     print(data)
    # break