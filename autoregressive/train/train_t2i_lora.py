    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for x, y, attn_mask, valid in tqdm(loader):
            ...
            if train_steps % args.ckpt_every == 0 and train_steps > 0:
                if rank == 0:
                    if not args.no_compile:
                        lora_model_weight = model.module._orig_mod.state_dict()
                    else:
                        lora_model_weight = model.module.state_dict() 
                        
                    # model.module.save_pretrained(os.path.join(checkpoint_dir,f"{train_steps:07d}","llamagen_lora"))
                    
                    for name in list(lora_model_weight.keys()):
                        if "lora" not in name:  # <-- adapt the condition to your use case
                            lora_model_weight.pop(name)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--lora-r", type=int, default=8, help="Rank of the LoRA update matrices")
    parser.add_argument("--lora-alpha", type=float, default=16, help="Scaling factor for the LoRA update")
