...
def main(args):
    ...
    model = GPT_models[args.gpt_model](
        ...
        cls_token_num=120, #args.cls_token_num,
    ...
    if args.only_train_ffn:
        for name, param in model.named_parameters():
            if 'w1' in name or 'w2' in name or 'w3' in name or 'cls_embedding' in name:
    ...
    if args.only_train_attn:
        for name, param in model.named_parameters():
            if 'wqkv' in name or 'wo' in name or 'cls_embedding' in name:
                param.requires_grad = True
            else:
                param.requires_grad = False
    ...
    if args.dataset == 't2i':     # create and load model
        ...
        vq_model = torch.compile(vq_model,mode="reduce-overhead",fullgraph=True)
        del checkpoint        
...
#        optimizer.load_state_dict(checkpoint["optimizer"])
    ...
    token_num = args.cls_token_num
    model.cls_token_num = token_num
    caption_channels = 2048
    model.cls_embedding.uncond_embedding = torch.nn.Parameter(torch.randn(token_num, caption_channels) / caption_channels ** 0.5)
    # 2d rotary pos embedding
    grid_size = int(model.block_size ** 0.5)
    assert grid_size * grid_size == model.block_size
    from autoregressive.models.gpt import precompute_freqs_cis_2d
    model.freqs_cis = precompute_freqs_cis_2d(grid_size, model.config.dim // model.config.n_head, model.config.rope_base, model.cls_token_num)
        
    #update the uncond_embedding
    
    
    if not args.no_compile:
        logger.info("compiling the model... (may take several minutes)")
        #model = torch.compile(model,mode="reduce-overhead",fullgraph=True) # requires PyTorch 2.0    
        model = torch.compile(model,mode="max-autotune",fullgraph=True) # requires PyTorch 2.0    
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for x, y, attn_mask, valid in tqdm(loader):
            ...
            with torch.amp.autocast(device_type="cuda",dtype=ptdtype):  
                _, loss = model(cond_idx=c_indices, idx=z_indices[:,:-1], targets=z_indices, mask=attn_mask[:, :, :-1,:-1], valid=valid) # c_indices = text cond
...
if __name__ == "__main__":
    ...
    parser.add_argument("--only-train-ffn", action='store_true')
    parser.add_argument("--only-train-attn", action='store_true')
    # parser.add_argument("--short-t5-feat-path", type=str, default=None, help="short caption of t5_feat_path")
    parser.add_argument("--cloud-save-path", type=str, required=False,default="/home/yetian/Project/LlamaGen/train_logs", help='please specify a cloud disk path, if not, local path')
    ...
    # Add wandb project name argument
    parser.add_argument("--wandb-project", type=str, default="your_default_project_name", help="Weights & Biases project name")
